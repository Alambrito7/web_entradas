...
@section('title', 'Registrar Nueva Entrada')
...
<div class="row mb-3">
    <div class="col-12">
        <h2><i class="bi bi-calendar-plus"></i> Registrar Nueva Entrada Folclórica</h2>
        <p class="text-muted">Complete el formulario para registrar una nueva festividad cultural</p>
    </div>
</div>

<form action="{{ route('entradas.store') }}" method="POST" enctype="multipart/form-data" id="entradaForm">
    @csrf
            ...
            <div class="row">
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <select name="status" class="form-select @error('status') is-invalid @enderror" required>
                        <option value="">Seleccione...</option>
                        <option value="planificada" {{ old('status') == 'planificada' ? 'selected' : '' }}>
                            Planificada
                        </option>
                        <option value="en_curso" {{ old('status') == 'en_curso' ? 'selected' : '' }}>
                            En Curso
                        </option>
            ...
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="descripcion" class="form-control @error('descripcion') is-invalid @enderror" 
                              rows="3" placeholder="Descripción breve de la entrada folclórica">{{ old('descripcion') }}</textarea>
                    @error('descripcion')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
        ...
        <div class="section-body">
            <div class="mb-3">
                ...
                <textarea name="historia" class="form-control @error('historia') is-invalid @enderror" 
                          rows="6" placeholder="Origen, evolución histórica, contexto cultural...">{{ old('historia') }}</textarea>
                @error('historia')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
    ...
    <section class="card section-card">
        ...
        <div class="section-body">
            <div class="map-instructions">
                ...
                <ul class="mb-0 small">
                    <li>Seleccione un departamento arriba para centrar el mapa</li>
                    <li>Haga clic en el mapa para marcar la ubicación exacta del evento</li>
                    <li>Puede arrastrar el marcador para ajustar la posición</li>
                    <li>El mapa se actualizará automáticamente al cambiar de departamento</li>
                </ul>
            </div>
                ...
                <div class="row">
                    <div class="col-md-6">
                        ...
                        <input type="text" id="latitud_display" class="form-control form-control-sm" readonly 
                               placeholder="Haga clic en el mapa">
...
<script>
    ...
    let map = L.map('map').setView([-16.5000, -68.1500], 6);
    let marker = null;
    ...
    document.getElementById('entradaForm').addEventListener('submit', function(e) {
        ...
        const departamento = document.querySelector('[name="departamento"]').value;
        const fechaEvento = document.querySelector('[name="fecha_evento"]').value;
        const status = document.querySelector('[name="status"]').value;

        if (!nombre || !departamento || !fechaEvento || !status) {
            e.preventDefault();
