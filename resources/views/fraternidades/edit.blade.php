...
@section('title', 'Editar Fraternidad')
...
<div class="row mb-3">
    <div class="col-12">
        <h2><i class="bi bi-pencil-square"></i> Editar Fraternidad: {{ $fraternidade->nombre }}</h2>
        <p class="text-muted">Actualice la información de la fraternidad</p>
    </div>
</div>

<form action="{{ route('fraternidades.update', $fraternidade) }}" method="POST" id="fraternidadForm">
    @csrf
    @method('PUT')
            ...
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    <input type="text" name="nombre" class="form-control @error('nombre') is-invalid @enderror" 
                           value="{{ old('nombre', $fraternidade->nombre) }}" 
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <select name="danza_id" class="form-select @error('danza_id') is-invalid @enderror">
                        ...
                        @foreach($danzas as $danza)
                            <option value="{{ $danza->id }}" {{ old('danza_id', $fraternidade->danza_id) == $danza->id ? 'selected' : '' }}>
                                {{ $danza->nombre }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="fecha_fundacion" 
                           ...
                           value="{{ old('fecha_fundacion', $fraternidade->fecha_fundacion ? $fraternidade->fecha_fundacion->format('Y-m-d') : '') }}">
            ...
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    <input type="text" name="lema" 
                           ...
                           value="{{ old('lema', $fraternidade->lema) }}" 
            ...
            <div class="row">
                <div class="col-md-12 mb-3">
                    ...
                    <textarea name="descripcion" class="form-control @error('descripcion') is-invalid @enderror" 
                              rows="4" placeholder="Historia, características, tradiciones de la fraternidad...">{{ old('descripcion', $fraternidade->descripcion) }}</textarea>
                    @error('descripcion')
                        <div class="invalid-feedback">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-6 mb-3">
                    ...
                    <input type="text" name="telefono" 
                           ...
                           value="{{ old('telefono', $fraternidade->telefono) }}" 
                           pattern="[0-9]{8}"
                           maxlength="8"
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="email" name="correo_electronico" 
                           ...
                           value="{{ old('correo_electronico', $fraternidade->correo_electronico) }}" 
    ...
    <section class="card section-card">
        ...
        <div class="section-body">
            <button type="button" class="btn btn-warning btn-sm mb-3" id="addPasante">
                <i class="bi bi-plus-circle"></i> Agregar Pasante
            </button>
            <small class="d-block text-muted mb-3">
                Los pasantes actuales se reemplazarán con los que agregue aquí. Puede modificar, eliminar o agregar nuevos.
            </small>
    ...
    <section class="card section-card">
        ...
        <div class="section-body">
            ...
            @else
                @php
                    $entradasSeleccionadas = old('entradas', $fraternidade->entradas->pluck('id')->toArray());
                @endphp
                            ...
                            <div class="entrada-checkbox">
                                <div class="form-check">
                                    <input class="form-check-input" 
                                           ...
                                           {{ in_array($entrada->id, $entradasSeleccionadas) ? 'checked' : '' }}>
        ...
        <div class="d-flex justify-content-between align-items-center flex-wrap gap-2">
            <div>
                <span class="text-muted small">
                    <i class="bi bi-info-circle"></i> Editando: {{ $fraternidade->nombre }}
                </span>
            </div>
            <div>
                <a href="{{ route('fraternidades.show', $fraternidade) }}" class="btn btn-secondary me-2">
                    <i class="bi bi-x-circle"></i> Cancelar
                </a>
                <button type="submit" class="btn btn-warning">
                    <i class="bi bi-save"></i> Actualizar Fraternidad
                </button>
            </div>
        </div>
    </div>
...
<script>
    ...
    // Cargar pasantes existentes
    @foreach($fraternidade->pasantes as $pasante)
        pasanteCount++;
        const container{{ $pasante->id }} = document.getElementById('pasantesContainer');
        const div{{ $pasante->id }} = document.createElement('div');
        div{{ $pasante->id }}.className = 'pasante-item';
        div{{ $pasante->id }}.innerHTML = `
            <div class="pasante-number">${pasanteCount}</div>
            <input type="text" 
                   name="pasantes[]" 
                   class="form-control" 
                   value="{{ $pasante->nombre }}"
        ...
        `;
        container{{ $pasante->id }}.appendChild(div{{ $pasante->id }});
    @endforeach
    ...
    if (entradasCheckboxes.length > 0) {
        ...
        btnContainer.innerHTML = `
            <button type="button" class="btn btn-outline-warning btn-sm me-2" id="selectAllEntradas">
